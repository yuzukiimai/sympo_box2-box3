...
\begin{table}[h!]
  \centering
    ...
    \begin{tabular}{|l||c|c|c|}  \hline
      ...
      2D-LiDAR & URM-40LC-EW &  UTM-30LX-EW & None\\
      & (HOKUYO) & (HOKUYO) & \\ \hline
...
\figref{fig:map}は，自己位置推定用に使用する地図であり，基本的には作成時のものから手を加えていない．
\figref{fig:costmap}は，経路計画用に用いる地図である．\figref{fig:map}の自己位置推定用に用いる地図に
侵入不可能な領域を設定している．このようにすることで，ロボットは侵入不可能領域外で
経路を生成するようになる．さらに，領域を囲うことで，経路を計算する領域も削減するため，
コンピュータの負荷を減らすことが可能である．
これらの地図は，確認走行エリア，その先から信号あり横断歩道まで，公園エリアのそれぞれで作成した
地図を合成している．地図の合成および編集には，GIMP\cite{gimp}を用いた．
