...
各ボックスや配達先の認識には，YOLOv5\cite{yolo}を用いた．YOLOは画像を入力すると，対象物が
画像中のどこにあるかを表すバウンディングボックスと認識率を出力する．
本課題の各ボックスと配達先の認識ために，データセットを作成した．
作成したデータセットは\cite{nagashima}で公開している．
手順としては，まず ORNE-box2の上部に取り付けたカメラを使用し，現地と千葉工業大学津田沼キャンパスで
画像を収集した．2261 26
...
日照条件の変化によりデータセットとの差が発生したことが考えられる．
